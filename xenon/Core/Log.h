...
#ifndef __XENON_CORE_LOG_H__
#define __XENON_CORE_LOG_H__
...
namespace xenon {
  ...
  class PerThreadBufferedStreamBuf : public std::basic_streambuf<CharT, traits> {
    ...
    typedef std::map<xenon::uint64, buffer_type> lookup_table_type;
  ...
  class LogRuleSet {
    // The ruleset determines what log messages are sent to the XENON system log file
  ...
  public:
    ...
    // at level xenon::InfoMessage or higher priority.
    LogRuleSet();
    virtual ~LogRuleSet();
  ...
  /// own LogRuleSet.
  ///
  /// Important Note: You should access the system log using the xenon::xenon_log
  ...
  class Log : private boost::noncopyable {
    ...
    std::map<xenon::uint64, boost::shared_ptr<multi_ostream> > m_multi_ostreams;
  ...
  /// log_namespace.
  std::ostream& xenon_out( int log_level = xenon::InfoMessage,
  ...
  void set_debug_level( int log_level ) XENON_DEPRECATED;
  ...
  void set_output_stream( std::ostream& stream ) XENON_DEPRECATED;

} // namespace xenon

#endif // __XENON_CORE_LOG_H__
