...
namespace {
  static xenon::null_ostream g_null_ostream;
}

// ---------------------------------------------------
// Basic stream support
// ---------------------------------------------------
std::ostream& xenon::xenon_out( int log_level, std::string log_namespace ) {
  return xenon_log()(log_level, log_namespace);
}

void xenon::set_debug_level( int log_level ) {
  xenon_log().console_log().rule_set().add_rule(log_level, "console");
}

void xenon::set_output_stream( std::ostream& stream ) {
  xenon_log().set_console_stream(stream);
}

// ---------------------------------------------------
// LogInstance Methods
// ---------------------------------------------------
xenon::LogInstance::LogInstance(std::string log_filename, bool prepend_infostamp) : m_prepend_infostamp(prepend_infostamp) {
  ...
  if (! static_cast<std::ofstream*>(m_log_ostream_ptr)->is_open())
    xenon_throw(IOErr() << "Could not open log file " << log_filename << " for writing.");
...
xenon::LogInstance::LogInstance(std::ostream& log_ostream, bool prepend_infostamp) : m_log_stream(log_ostream),
                                                                                  m_log_ostream_ptr(NULL),
                                                                                  m_prepend_infostamp(prepend_infostamp) {}

std::ostream& xenon::LogInstance::operator() (int log_level, std::string log_namespace) {
...
std::ostream& xenon::Log::operator() (int log_level, std::string log_namespace) {
  // First, check to see if the rc file has been updated.
  // Reload the rulesets if it has.
  xenon_settings().reload_config();
...
xenon::LogRuleSet::LogRuleSet( LogRuleSet const& copy_log) {
  m_rules = copy_log.m_rules;
}

xenon::LogRuleSet& xenon::LogRuleSet::operator=( LogRuleSet const& copy_log) {
  m_rules = copy_log.m_rules;
  return *this;
}


xenon::LogRuleSet::LogRuleSet() { }
xenon::LogRuleSet::~LogRuleSet() { }

void xenon::LogRuleSet::add_rule(int log_level, std::string log_namespace) {
  ssize_t count = std::count(log_namespace.begin(), log_namespace.end(), '*');
  if (count > 1)
    xenon::xenon_throw(xenon::ArgumentErr() << "Illegal log rule: only one wildcard is supported.");

  if (count == 1 && *(log_namespace.begin()) != '*'
                 && *(log_namespace.end()-1) != '*')
    xenon::xenon_throw(xenon::ArgumentErr() << "Illegal log rule: wildcards must be at the beginning or end of a rule");

  Mutex::Lock lock(m_mutex);
  m_rules.push_front(rule_type(log_level, boost::to_lower_copy(log_namespace)));
}

void xenon::LogRuleSet::clear() {
  Mutex::Lock lock(m_mutex);
  m_rules.clear();
}
...
bool xenon::LogRuleSet::operator() (int log_level, std::string log_namespace) {
  Mutex::Lock lock(m_mutex);
  ...
  for (rules_type::iterator it = m_rules.begin(); it != m_rules.end(); ++it) {
    ...
    if (rule_lvl == xenon::EveryMessage)
      return true;

    return log_level <= rule_lvl;
  }

  if (log_level <= xenon::InfoMessage)
    if (log_namespace == "console" || wildcard_match("*.progress", lower_namespace))
      return true;
  if (log_level <= xenon::WarningMessage)
    return true;
