...
/// set using the ~/.xenonrc file in the user's home directory.  That
...
#ifndef __XENON_CORE_SETTINGS_H__
#define __XENON_CORE_SETTINGS_H__
...
namespace xenon {
  ...
  /// static xenon_settings() function below, which returns a singleton
...
#define XENON_DECLARE_SETTING(Name, Type)\
    private:\
      Type m_ ## Name; \
    ...
    XENON_DECLARE_SETTING(write_pool_size, uint32);

    // The default tile size (in pixels) used for block processing ops.
    XENON_DECLARE_SETTING(default_tile_size, uint32);

    // The directory used to store temporary files.
    XENON_DECLARE_SETTING(tmp_directory, std::string);

#undef XENON_DECLARE_SETTING
  ...
  public:
    ...
    /// xenon_settings() method below.
    Settings();

    /// Change the rc filename (default: ~/.xenonrc)
...
#endif // __XENON_CORE_SETTINGS_H__
