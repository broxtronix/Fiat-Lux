...
#include <xenon/Core/Debugging.h>

void xenon::Cache::allocate( size_t size ) {
  while( m_size+size > m_max_size ) {
    if( ! m_last_valid ) {
      xenon_out(WarningMessage, "console") << "Warning: Cached object (" << size << ") larger than requested maximum cache size (" << m_max_size << "). Current Size = " << m_size << "\n";
      xenon_out(WarningMessage, "cache") << "Warning: Cached object (" << size << ") larger than requested maximum cache size (" << m_max_size << "). Current Size = " << m_size << "\n";
      break;
    }
  ...
  m_size += size;
  XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache allocated " << size << " bytes (" << m_size << " / " << m_max_size << " used)" << "\n"; )
}

void xenon::Cache::resize( size_t size ) {
  Mutex::Lock lock(m_mutex);
  m_max_size = size;
  while( m_size > m_max_size ) {
    XENON_ASSERT( m_last_valid, LogicErr() << "Cache is empty but has nonzero size!" );
    m_last_valid->invalidate();
  }
}

void xenon::Cache::deallocate( size_t size ) {
  m_size -= size;
  XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache deallocated " << size << " bytes (" << m_size << " / " << m_max_size << " used)" << "\n"; )
}

// Move the cache line to the top of the valid list.
void xenon::Cache::validate( CacheLineBase *line ) {
...
void xenon::Cache::invalidate( CacheLineBase *line ) {
...
void xenon::Cache::remove( CacheLineBase *line ) {
  if( line == m_first_valid ) m_first_valid = line->m_next;
  if( line == m_last_valid ) m_last_valid = line->m_prev;
  if( line == m_first_invalid ) m_first_invalid = line->m_next;
  if( line->m_next ) line->m_next->m_prev = line->m_prev;
...
void xenon::Cache::deprioritize( CacheLineBase *line ) {
