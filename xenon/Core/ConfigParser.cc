...
void xenon::parse_config_file(const char* fn, xenon::Settings& settings) {
  std::ifstream file(fn);

  if (!file.is_open())
    xenon_throw(IOErr() <<  "Could not open logfile: " << fn);

  parse_config(file, settings);
}

void xenon::parse_config(std::basic_istream<char>& stream,
                      xenon::Settings& settings) {

  // DO NOT try to log with xenon_log! It will cause a deadlock because we're
  ...
  try {
    ...
    std::cerr << "Could not parse config file. Ignoring. ("
      ...
      xenon_throw(LogicErr() << "We should be accepting all options. This shouldn't happen.");
  }
    ...
    try {
      if (o.string_key == "general.default_num_threads")
        ...
        if (current_log)
          ...
          xenon_log().console_log().rule_set().add_rule(level, domain);
      }
      else {
