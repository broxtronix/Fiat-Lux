...
///
#ifndef __XENON_CORE_THREADPOOL_H__
#define __XENON_CORE_THREADPOOL_H__
...
namespace xenon {
  ...
  class WorkQueue {
    ...
    public:
      ...
      void operator()() {
        do {
          xenon_out(DebugMessage, "thread") << "ThreadPool: running worker thread "
                                         << m_thread_id << "\n";
          (*m_task)();
    ...
    void worker_thread_complete(int worker_id) {
      m_active_workers--;
      xenon_out(DebugMessage, "thread") << "ThreadPool: terminating worker thread " << worker_id << ".  [ " << m_active_workers << " / " << m_max_workers << " now active ]\n";

      // Erase the worker thread from the list of active threads
      XENON_ASSERT(worker_id >= 0 && worker_id < int(m_running_threads.size()),
  ...
  public:
    WorkQueue(int num_threads = xenon_settings().default_num_threads() )
    ...
    void notify() {
      ...
      while ( !m_available_thread_ids.empty() &&
        ...
        m_active_workers++;
        xenon_out(DebugMessage, "thread") << "ThreadPool: creating worker thread " << next_available_thread_id << ".  [ " << m_active_workers << " / " << m_max_workers << " now active ]\n";
      }
    }
  ...
  public:
    ...
    FifoWorkQueue(int num_threads = xenon_settings().default_num_threads()) : WorkQueue(num_threads) {}
  ...
  public:
    ...
    OrderedWorkQueue(int num_threads = xenon_settings().default_num_threads()) : WorkQueue(num_threads) {
      m_next_index = 0;
    }
...
#endif // __XENON_CORE_THREADPOOL_H__
