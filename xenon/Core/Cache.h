...
/// The xenon::Cache object defined in src/xenon/Core/Cache.h can be used to
...
/// the result.
///
#ifndef __XENON_CORE_CACHE_H__
#define __XENON_CORE_CACHE_H__

// Uncomment one of these to enable or disable cache debug messages
#define XENON_CACHE_DEBUG(x) x
//#define XENON_CACHE_DEBUG(x)
...
namespace xenon {
  ...
  class Cache {
    ...
    public:
      ...
      {
        XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache creating CacheLine " << info() << "\n"; )
      ...
      virtual ~CacheLine() {
        Mutex::Lock cache_lock(cache().m_mutex);
        invalidate();
        XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache destroying CacheLine " << info() << "\n"; )
        remove();
      }
      ...
      virtual void invalidate() {
        ...
        if( ! m_value ) return;
        XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache invalidating CacheLine " << info() << "\n"; );
        CacheLineBase::invalidate();
      ...
      value_type const& value() {
        ...
        if( !m_value ) {
          m_generation_count++;
          hit = false;
          XENON_CACHE_DEBUG( xenon_out(DebugMessage, "cache") << "Cache generating CacheLine " << info() << "\n"; )
          {
            Mutex::Lock cache_lock(cache().m_mutex);
    ...
    Mutex m_mutex;
    xenon::uint64 m_hits, m_misses, m_evictions;
  ...
  public:
    ...
    public:
      ...
      boost::shared_ptr<value_type> operator->() const {
        XENON_ASSERT( m_line_ptr, NullPtrErr() << "Invalid cache handle!" );
    ...
    Handle<GeneratorT> insert( GeneratorT const& generator ) {
      ...
      XENON_ASSERT( line, NullPtrErr() << "Error creating new cache line!" );
