...
///
#ifndef __XENON_CORE_PROGRESSCALLBACK_H__
#define __XENON_CORE_PROGRESSCALLBACK_H__
...
namespace xenon {
  ...
  public:
    ...
    // Throw xenon::Aborted if abort has been requested
  ...
  class TerminalProgressCallback : public ProgressCallback {
    ...
    void calculate_bar_length() {
      XENON_ASSERT( m_pre_progress_text.size()+8+m_precision < 80,
  ...
  public:
    TerminalProgressCallback( MessageLevel level = InfoMessage, std::string pre_progress_text = "", uint32_t precision = 0 ) XENON_DEPRECATED;
