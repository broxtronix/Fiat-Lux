...
void lux::SimulatorEngine::draw_gl() {
  ...
  for (int i = 0; i<LUX_SIMULATOR_HIST_SAMPLES; i++) {
    ...
    // Lowpass to simulate slow galvo tracking rate
    s.x = last_x * 0.65 + s.x * 0.35;
    s.y = last_y * 0.65 + s.y * 0.35;

    // Delay brightness changes
    rdelay[i%2] = s.r;
    gdelay[i%2] = s.g;
    bdelay[i%2] = s.b;
    s.r = rdelay[(i+1)%2];
    ...
    if (fabsf(s.x-last_x) < 0.001 && fabsf(s.y-last_y) < 0.001) {
      ...
      // If the galvo position has been updated, draw a line
      // connecting the points.
      r = (s.r-0.2) * factor * dfactor * 1.8;
      g = (s.g-0.2) * factor * dfactor * 1.8;
      b = (s.b-0.2) * factor * dfactor * 1.8;
      glBegin(GL_LINES);
      laser_color(last_r, last_g, last_b, 0.8);
      glVertex3f(last_x, last_y, 0);
