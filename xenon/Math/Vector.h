...
///
#ifndef __XENON_MATH_VECTOR_H__
#define __XENON_MATH_VECTOR_H__
...
#include <xenon/Core/FundamentalTypes.h>
#include <xenon/Core/VarArray.h>
#include <xenon/Core/Exception.h>
#include <xenon/Core/Functors.h>
#include <xenon/Math/Functors.h>
...
namespace math {
  ...
  public:
    ...
    /// proxy object instead, see xenon::VectorProxy.
    ...
    /// Generalized copy constructor, from arbitrary XENON vector expressions.
    template <class T>
    Vector( VectorBase<T> const& v ) {
      XENON_ASSERT( v.impl().size()==SizeN, ArgumentErr() << "Vector must have dimension " << SizeN << "." );
    ...
    /// Generalized assignment operator, from arbitrary XENON vector expressions.
    template <class T>
    ...
    void set_size( size_t new_size, bool /*preserve*/ = false ) {
      XENON_ASSERT( new_size==size(), ArgumentErr() << "Cannot change size of fixed-size Vector." );
    }
  ...
  public:
    ...
    VectorProxy& operator=( VectorProxy const& v ) {
      XENON_ASSERT( v.size()==size(), ArgumentErr() << "Vector must have dimension " << size() << " in vector proxy assignment." );
    ...
    VectorProxy& operator=( VectorBase<T> const& v ) {
      XENON_ASSERT( v.impl().size()==size(), ArgumentErr() << "Vector must have dimension " << size() << " in vector proxy assignment." );
      Vector<value_type> tmp( v );
      VectorAssignImpl<VectorProxy,Vector<value_type> >::assign(*this,tmp);
      return *this;
    }
    ...
    /// Change the size of the vector. Elements in memory are preserved when specified.
    void set_size( size_t new_size, bool /*preserve*/ = false ) {
      XENON_ASSERT( new_size==size(), ArgumentErr() << "Cannot resize a vector proxy." );
    }
  ...
  public:
    ...
    VectorTranspose& operator=( VectorTranspose<OtherT> const& v ) {
      XENON_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in transposed vector assignment" );
      m_vector = v.child();
      return *this;
    }
  ...
  public:
    ...
    SubVector& operator=( SubVector const& v ) {
      XENON_ASSERT( v.size()==size(), ArgumentErr() << "Vectors must have same size in subvector assignment" );
    ...
    SubVector& operator=( VectorBase<OtherT> const& v ) {
      XENON_ASSERT( v.impl().size()==m_size, ArgumentErr() << "Vectors must have same size in subvector assignment" );
  ...
  public:
    ...
    VectorT const& child() const { return v; }
  ...
  public:
    ...
    VectorBinaryFunc( Vector1T const& v1, Vector2T const& v2 ) : v1(v1), v2(v2), func() {
      XENON_ASSERT( v1.size() == v2.size(), ArgumentErr() << "Vectors must have same size in VectorBinaryFunc" );
    }
    ...
    Vector1T const& child1() const { return v1; }
    Vector2T const& child2() const { return v2; }
    size_t size() const { return child1().size(); }

    const_reference_type operator()( size_t i ) const { return func(child1()(i),child2()(i)); }
    const_reference_type operator[]( size_t i ) const { return func(child1()[i],child2()[i]); }
  ...
  /// you will have to explicitly request this one as xenon::math::equal().
  template <class Vector1T, class Vector2T>
