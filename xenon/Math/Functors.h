...
///
#ifndef __XENON_MATH_FUNCTORS_H__
#define __XENON_MATH_FUNCTORS_H__
...
#include <xenon/Core/CompoundTypes.h>
#include <xenon/Core/TypeDeduction.h>
#include <xenon/Core/Functors.h>
...
// double XENON math routines here for certain platforms.
#if defined(__FreeBSD__) || defined(_WIN32)
#define __XENON_MATH_DISABLE_LONG_DOUBLE_ARITHMETIC
#endif
...
#if !defined(__XENON_MATH_DISABLE_LONG_DOUBLE_ARITHMETIC)
  // This will actually create a specialization for long doubles
  #define __XENON_MATH_UNARY_LONG_DOUBLE_IMPL(func)                         \
      long double operator()( long double arg ) const {                  \
        return func##l(arg);                                             \
      }
  #define __XENON_MATH_BINARY_LONG_DOUBLE_IMPL(func)                        \
...
#define __XENON_UNARY_MATH_FUNCTOR(name,func)                              \
  struct Arg##name##Functor                                             \
    ...
    __XENON_MATH_UNARY_LONG_DOUBLE_IMPL(func)                              \
  };                                                                    \
  using ::func;

#define __XENON_BINARY_MATH_FUNCTOR(name,func)                             \
  struct ArgArg##name##Functor                                          \
    ...
    __XENON_MATH_BINARY_LONG_DOUBLE_IMPL(func)                             \
  ...
  __XENON_UNARY_MATH_FUNCTOR( Fabs, fabs )
  __XENON_UNARY_MATH_FUNCTOR( Acos, acos )
  __XENON_UNARY_MATH_FUNCTOR( Asin, asin )
  __XENON_UNARY_MATH_FUNCTOR( Atan, atan )
  __XENON_UNARY_MATH_FUNCTOR( Cos, cos )
  __XENON_UNARY_MATH_FUNCTOR( Sin, sin )
  __XENON_UNARY_MATH_FUNCTOR( Tan, tan )
  __XENON_UNARY_MATH_FUNCTOR( Cosh, cosh )
  __XENON_UNARY_MATH_FUNCTOR( Sinh, sinh )
  __XENON_UNARY_MATH_FUNCTOR( Tanh, tanh )
  __XENON_UNARY_MATH_FUNCTOR( Exp, exp )
  __XENON_UNARY_MATH_FUNCTOR( Log, log )
  __XENON_UNARY_MATH_FUNCTOR( Log10, log10 )
  __XENON_UNARY_MATH_FUNCTOR( Sqrt, sqrt )
  __XENON_UNARY_MATH_FUNCTOR( Ceil, ceil )
  __XENON_UNARY_MATH_FUNCTOR( Floor, floor )

  __XENON_BINARY_MATH_FUNCTOR( Atan2, atan2 )
  __XENON_BINARY_MATH_FUNCTOR( Pow, pow )
  __XENON_BINARY_MATH_FUNCTOR( Hypot, hypot )

#ifndef WIN32
  __XENON_UNARY_MATH_FUNCTOR( Acosh, acosh )
  __XENON_UNARY_MATH_FUNCTOR( Asinh, asinh )
  __XENON_UNARY_MATH_FUNCTOR( Atanh, atanh )

#ifdef XENON_HAVE_EXP2
  __XENON_UNARY_MATH_FUNCTOR( Exp2, exp2 )
#endif
#ifdef XENON_HAVE_LOG2
  __XENON_UNARY_MATH_FUNCTOR( Log2, log2 )
#endif
#ifdef XENON_HAVE_TGAMMA
  __XENON_UNARY_MATH_FUNCTOR( Tgamma, tgamma )
#endif
  __XENON_UNARY_MATH_FUNCTOR( Lgamma, lgamma )
  __XENON_UNARY_MATH_FUNCTOR( Expm1, expm1 )
  __XENON_UNARY_MATH_FUNCTOR( Log1p, log1p )
  __XENON_UNARY_MATH_FUNCTOR( Cbrt, cbrt )
  __XENON_UNARY_MATH_FUNCTOR( Erf, erf )
  __XENON_UNARY_MATH_FUNCTOR( Erfc, erfc )
  __XENON_UNARY_MATH_FUNCTOR( Round, round )
  __XENON_UNARY_MATH_FUNCTOR( Trunc, trunc )

  __XENON_BINARY_MATH_FUNCTOR( Copysign, copysign )
  __XENON_BINARY_MATH_FUNCTOR( Fdim, fdim )
#endif

#undef __XENON_UNARY_MATH_FUNCTOR
#undef __XENON_BINARY_MATH_FUNCTOR
  ...
  public:
    ...
    ValT minimum() const {
      XENON_ASSERT(m_valid, ArgumentErr() << "MinMaxAccumulator: no valid samples");
      return m_minval;
    }
  ...
  public:
    ...
    ValT const& value() {
      XENON_ASSERT(m_values.size(), ArgumentErr() << "MedianAccumulator: no valid samples");
  ...
  public:
    ...
    value_type value() const {
      XENON_ASSERT(m_count, ArgumentErr() << "MeanAccumulator: no valid samples");
  ...
  public:
    ...
    value_type value() const {
      XENON_ASSERT(num_samples, ArgumentErr() << "StdDevAccumulator(): no valid samples.");
  ...
  public:
    ...
    void resize( size_t buffersize, size_t quantiles ) {
      XENON_ASSERT(quantiles > 0, LogicErr() << "Cannot have 0 quantiles");
...
#endif  // __XENON_MATH_FUNCTORS_H__
